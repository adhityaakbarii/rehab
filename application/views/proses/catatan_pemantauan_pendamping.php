		...
		<div class="container-fluid">
			...
			if($set=="insert"){
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card">
							<form class="form-horizontal" action="proses/catatan_pemantauan_pendamping/<?php echo $form ?>" method="POST" enctype="multipart/form-data">
								<div class="card-header">
									<h5>Catatan Pemantauan Pendamping</h5>
								</div>
								<div class="card-body">								
									...
									<div class="form-group row" style="margin-left: 1px;">
										<label for="fname" class="col-sm-3 text-left control-label col-form-label">4. Catatan Pemantauan Pendamping </label>
										<label for="fname" class="col-sm-1 text-right control-label col-form-label">:</label>
										<div class="col-sm-8">
									...
									<div class="form-group row">
										<label for="fname" class="col-sm-12 text-left control-label col-form-label"><a href="proses/catatan_perkembangan/add"><i class="fa fa-angle-double-left" aria-hidden="true"></i> Sebelumnya</a></label>
									</div>
							...
							<table id="zero_config" class="table table-striped table-bordered">
								...
								<tbody>
									...
									foreach ($dt_rawat_jalan_2->result() as $row) {
										...
										<tr>
										<td>$no</td>
										<td><a href='proses/catatan_pemantauan_pendamping/detail?id=$row->id_staf'>$row->id</a></td>
										...
										<td>$row->pekerjaan</td>
										<td>"; ?>
										<a class='btn btn-info btn-sm' href='proses/catatan_pemantauan_pendamping/edit?id=<?php echo $row->id_staf ?>'><i class='fa fa-edit'></i></a>
										<a class='btn btn-danger btn-sm' onclick="return confirm('Anda yakin?')" href='proses/catatan_pemantauan_pendamping/delete?id=<?php echo $row->id_staf ?>'><i class='fa fa-trash'></i></a>
									</td>
								</tr>
								<?php
								$no++;
...
<script type="text/javascript">
	function ambil_kab(){  
		...
		$.ajax({
			url : "<?php echo site_url('proses/catatan_pemantauan_pendamping/ambil_kab')?>",
			type:"POST",
	...
	function ambil_kec(){  
		...
		$.ajax({
			url : "<?php echo site_url('proses/catatan_pemantauan_pendamping/ambil_kec')?>",
			type:"POST",
	...
	function ambil_kel(){  
		...
		$.ajax({
			url : "<?php echo site_url('proses/catatan_pemantauan_pendamping/ambil_kel')?>",
			type:"POST",
