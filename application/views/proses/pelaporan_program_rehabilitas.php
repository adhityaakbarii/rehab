		...
		<div class="container-fluid">
			...
			if($set=="insert"){
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card">
							<form class="form-horizontal" action="proses/pelaporan_program_rehabilitas/<?php echo $form ?>" method="POST" enctype="multipart/form-data">
							...
							<div class="card-body">						
								<div class="form-group row">
									...
									<div class="col-sm-10">										
										<input type="text" <?php echo $read ?> value="<?php echo $tampil = ($row!='') ? $row->nama_assesmen : "" ; ?>" name="nama_assesmen" class="form-control" placeholder="Nama Klien">
									</div>									
								</div>
								<div class="form-group row">
								...
								<div class="form-group row">									
									...
									<label for="fname" class="col-sm-2 text-right control-label col-form-label">Jenis laporan_assesmen</label>
									<div class="col-sm-4">
										<select onclick="cek_jenis()" id="jenis_assesmen" name="jenis_assesmen" class="form-control" <?php echo $read2 ?>>
											<option <?php if($row!='' && $row->jenis_assesmen=="") echo "selected" ?> value=''>- pilih -</option>
											<option <?php if($row!='' && $row->jenis_assesmen=="Compulsory") echo "selected" ?> >Compulsory</option>
											<option <?php if($row!='' && $row->jenis_assesmen=="Voluntary") echo "selected" ?> >Voluntary</option>												
										</select>
									</div>								
								</div>
								<div class="form-group row">
									...
									<div class="table-responsive">
										<table id="zero_config" class="table table-striped table-bordered">
											...
											<tbody>
												<?php 
												$no=1;
												foreach ($dt_pelaporan_program_rehabilitas->result() as $row) {
													echo "
													<tr>
													<td>$no</td>
													<td><a href='proses/pelaporan_program_rehabilitas/detail?id=$row->id_staf'>$row->id</a></td>
													...
													<td>$row->pekerjaan</td>
													<td>"; ?>
													<a class='btn btn-info btn-sm' href='proses/pelaporan_program_rehabilitas/edit?id=<?php echo $row->id_staf ?>'><i class='fa fa-edit'></i></a>
													<a class='btn btn-danger btn-sm' onclick="return confirm('Anda yakin?')" href='proses/pelaporan_program_rehabilitas/delete?id=<?php echo $row->id_staf ?>'><i class='fa fa-trash'></i></a>
												</td>
											</tr>
											<?php
											$no++;
...
<script type="text/javascript">
	function ambil_kab(){  
		...
		$.ajax({
			url : "<?php echo site_url('proses/pelaporan_program_rehabilitas/ambil_kab')?>",
			type:"POST",
	...
	function ambil_kec(){  
		...
		$.ajax({
			url : "<?php echo site_url('proses/pelaporan_program_rehabilitas/ambil_kec')?>",
			type:"POST",
	...
	function ambil_kel(){  
		...
		$.ajax({
			url : "<?php echo site_url('proses/pelaporan_program_rehabilitas/ambil_kel')?>",
			type:"POST",
