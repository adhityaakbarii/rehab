<body onload="sembunyi()">
		...
		<div class="container-fluid">
			...
			if($set=="insert"){
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card">
							<form class="form-horizontal" action="proses/client/<?php echo $form ?>" method="POST" enctype="multipart/form-data">
								...
								<div class="card-body">								
									<div class="form-group row">									
										...
										<div class="col-sm-4">
											<?php echo $form_id ?>
											<select name="id_instansi" class="select2 form-control custom-select" <?php echo $read2 ?>>
								...
								<div class="card-body">						
									<div class="form-group row">
										...
										<div class="col-sm-10">										
											<input type="text" <?php echo $read ?> value="<?php echo $tampil = ($row!='') ? $row->nama_client : "" ; ?>" name="nama_client" class="form-control" placeholder="Nama Klien">
										</div>									
									</div>
									<div class="form-group row">
									...
									<div class="form-group row">									
										...
										<label for="fname" class="col-sm-2 text-right control-label col-form-label">Jenis Client</label>
										<div class="col-sm-4">
											<select onclick="cek_jenis()" id="jenis_client" name="jenis_client" class="form-control" <?php echo $read2 ?>>
												<option <?php if($row!='' && $row->jenis_client=="") echo "selected" ?> value=''>- pilih -</option>
												<option <?php if($row!='' && $row->jenis_client=="Compulsory") echo "selected" ?> >Compulsory</option>
												<option <?php if($row!='' && $row->jenis_client=="Voluntary") echo "selected" ?> >Voluntary</option>												
											</select>
										</div>								
									</div>
									<div class="form-group row">
										...
										<div class="table-responsive">
											<table id="zero_config" class="table table-striped table-bordered">
												...
												<tbody>
													<?php 
													$no=1;
													foreach ($dt_client->result() as $row) {
														echo "
														<tr>
														<td>$no</td>
														<td><a href='proses/client/detail?id=$row->id_staf'>$row->id</a></td>
														...
														<td>$row->pekerjaan</td>
														<td>"; ?>
														<a class='btn btn-info btn-sm' href='proses/client/edit?id=<?php echo $row->id_staf ?>'><i class='fa fa-edit'></i></a>
														<a class='btn btn-danger btn-sm' onclick="return confirm('Anda yakin?')" href='proses/client/delete?id=<?php echo $row->id_staf ?>'><i class='fa fa-trash'></i></a>
													</td>
												</tr>
												<?php
												$no++;
	...
	<script type="text/javascript">
		function ambil_kab(){  
			...
			$.ajax({
				url : "<?php echo site_url('proses/client/ambil_kab')?>",
				type:"POST",
		...
		function ambil_kec(){  
			...
			$.ajax({
				url : "<?php echo site_url('proses/client/ambil_kec')?>",
				type:"POST",
		...
		function ambil_kel(){  
			...
			$.ajax({
				url : "<?php echo site_url('proses/client/ambil_kel')?>",
				type:"POST",
		...
		function cek_jenis(){  
			var jenis_client  = $("#jenis_client").val();   
			if(jenis_client=='Compulsory'){
				$("#tampil_jenis").show();						
			}else{
				$("#tampil_jenis").show();
			}
